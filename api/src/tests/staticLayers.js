...
describe('GET /api/staticLayers', () => {
  ...
  it('should return all layers in db sorted by title with geoDataUrl and format from layerGeoData collection', async () => {
    ...
    expect(res.body[0].metadata.geoMetadata).toEqual(mapLayersInDb[0].metadata.geoMetadata);
  });
  it('should return layer of type group with correct geodata links and format in sublayers', async () => {
    ...
    expect(res.body[0].metadata.geoMetadata).toEqual(mapLayersInDb[2].metadata.geoMetadata);
